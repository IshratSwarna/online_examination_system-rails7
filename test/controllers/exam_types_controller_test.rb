...
class ExamTypesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create exam_type" do
    assert_difference("ExamType.count") do
      post exam_types_url, params: { exam_type: { name: @exam_type.name, status: @exam_type.status } }
    end
  ...
  test "should update exam_type" do
    patch exam_type_url(@exam_type), params: { exam_type: { name: @exam_type.name, status: @exam_type.status } }
